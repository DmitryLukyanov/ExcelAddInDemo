...
public class NativeHooks
{
    ...
    private static Action<Func<Keys, bool>>[] _actions;

    public static void SetHook(params Action<Func<Keys, bool>>[] actions)
    {
...
#pragma warning restore 618
        ...
        _actions = actions ?? throw new ArgumentNullException("Actions must be not null.");
    }
    ...
    private static int HookCallback(int nCode, IntPtr wParam, IntPtr lParam)
    {
        ...
        else
        {
            ...
            if (nCode == HC_ACTION)
            {
                ...
                {
                    action((k) => BindingFunctions.IsKeyDown(k));
                }
            }
            return (int)CallNextHookEx(_hookID, nCode, wParam, lParam);
        }
